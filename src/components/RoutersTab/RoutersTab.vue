<template>
  <div class="ui_content_tab" :style="bg2">
    <el-row @mouseout.native="MouseOutRow()">
      <el-col :span="3" v-for="(item,index) in navArr" :key="index" @mouseover.native="FirstMouseOver(index)"
              @click.native="FirstClick(index)">
        <div class="ui_nav_style ellipsis"
             :style="{'background':item.activeBgColor}"
             :title="item.name">{{item.name}}
          <!--    二级菜单-->
          <el-row class="ui_second_row_nav" :class="{'activeBlock': current === index}"
                  @mouseout.native="SecondRowMouseOut()">
            <el-col :span="3" v-for="(ite,idx) in item.children" :key="idx" @click.native="SecondClick(idx)"
                    @mouseover.native="SecondMouseOver(idx)">
